...
public class SQLiteHelper extends SQLiteOpenHelper {
    ...
    public void insertRecord(ContactModel contact) {
        ...
        contentValues.put(COLUMN_FIRST_NAME, contact.getNome());
        contentValues.put(COLUMN_LAST_NAME, contact.getPreco());
    ...
    public void insertRecordAlternate(ContactModel contact) {
        database = this.getReadableDatabase();
        database.execSQL("INSERT INTO " + TABLE_NAME + "(" + COLUMN_FIRST_NAME + "," + COLUMN_LAST_NAME + ") VALUES('" + contact.getNome() + "','" + contact.getPreco() + "')");
        database.close();
    }
    ...
    public ArrayList<ContactModel> getAllRecords() {
        ...
        if (cursor.getCount() > 0) {
            for (int i = 0; i < cursor.getCount(); i++) {
                ...
                contactModel.setNome(cursor.getString(1));
                contactModel.setPreco(cursor.getString(2));
    ...
    public void updateRecordAlternate(ContactModel contact) {
        database = this.getReadableDatabase();
        database.execSQL("update " + TABLE_NAME + " set " + COLUMN_FIRST_NAME + " = '" + contact.getNome() + "', " + COLUMN_LAST_NAME + " = '" + contact.getPreco() + "' where " + COLUMN_ID + " = '" + contact.getID() + "'");
        database.close();
    }
